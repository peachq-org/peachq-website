...
// Missing metadata must not present an old release as current.
$release = '';
$releaseDisplay = '';
$uploaded = '';
$files = array_fill_keys(['windows', 'mac', 'linux', 'windows_duckdb',
                         'macos_duckdb', 'linux_glibc_duckdb'], '');
$releasesUrl = 'https://github.com/peachq-org/peachq/releases';
...
if (is_readable($latestPath)) {
    ...
    if (is_array($latest)) {
        if (!empty($latest['version']) && is_string($latest['version'])) {
            ...
            // Display the short version while retaining the manifest's release tag.
        ...
        foreach (array_keys($files) as $platform) {
...
$commands = [
    'linux' => "curl -fLO https://peachq.org/download/peachq-linux-x64.tar.gz\nmkdir -p peachq\ntar -xzf peachq-linux-x64.tar.gz -C peachq\ncd peachq\n./q",
    'mac' => "curl -fLO https://peachq.org/download/peachq-mac-arm64.tar.gz\nmkdir -p peachq\ntar -xzf peachq-mac-arm64.tar.gz -C peachq\ncd peachq\n./q",
    'windows' => "Invoke-WebRequest https://peachq.org/download/peachq.zip -OutFile peachq.zip\nExpand-Archive peachq.zip -DestinationPath peachq\ncd peachq\n.\\q.exe",
];
?>
<main><section class="page-hero"><div class="container"><span class="tag">Preview builds</span><h1>Download Peach<span class="peach">Q</span>.</h1><p>Native packages for the current preview, plus the browser REPL for a no-install first look.</p><p class="release-meta" data-release-summary<?= $release === '' ? ' hidden' : '' ?>>Current preview <strong data-release-version><?= htmlspecialchars($releaseDisplay, ENT_QUOTES, 'UTF-8') ?></strong> uploaded <time data-release-uploaded datetime="<?= htmlspecialchars($uploaded, ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($uploaded, ENT_QUOTES, 'UTF-8') ?></time>.</p><p data-release-unavailable<?= $release !== '' ? ' hidden' : '' ?>>Release information is unavailable. See <a href="<?= $releasesUrl ?>">published releases</a>.</p></div></section>
<section class="section"><div class="container">
<div class="download-grid" data-platform-downloads>
<div class="card download-card" data-platform-card="windows"><div class="os">⊞</div><span class="tag">Windows x64</span><h3>Windows</h3><p>Standalone preview archive for 64-bit Windows.</p><div class="file-name" data-release-file="windows"><?= htmlspecialchars($files['windows'], ENT_QUOTES, 'UTF-8') ?></div><div class="file-meta" data-release-meta="windows" hidden></div><div class="actions"><a class="button peach-button" data-release-link="windows" href="<?= $files['windows'] !== '' ? 'file/' . rawurlencode($files['windows']) : $releasesUrl ?>">Download Windows</a></div>
<p class="download-alternative" data-release-optional="windows_duckdb"<?= $files['windows_duckdb'] === '' ? ' hidden' : '' ?>><small><a data-release-link="windows_duckdb" href="<?= $files['windows_duckdb'] !== '' ? 'file/' . rawurlencode($files['windows_duckdb']) : $releasesUrl ?>">Download with DuckDB</a></small></p></div>
<div class="card download-card" data-platform-card="mac"><div class="os">◉</div><span class="tag">macOS arm64</span><h3>macOS</h3><p>Apple silicon preview archive with the PeachQ command-line binary.</p><div class="file-name" data-release-file="mac"><?= htmlspecialchars($files['mac'], ENT_QUOTES, 'UTF-8') ?></div><div class="file-meta" data-release-meta="mac" hidden></div><div class="actions"><a class="button peach-button" data-release-link="mac" href="<?= $files['mac'] !== '' ? 'file/' . rawurlencode($files['mac']) : $releasesUrl ?>">Download macOS</a></div>
<p class="download-alternative" data-release-optional="macos_duckdb"<?= $files['macos_duckdb'] === '' ? ' hidden' : '' ?>><small><a data-release-link="macos_duckdb" href="<?= $files['macos_duckdb'] !== '' ? 'file/' . rawurlencode($files['macos_duckdb']) : $releasesUrl ?>">Download with DuckDB</a></small></p></div>
<div class="card download-card" data-platform-card="linux"><div class="os">🐧</div><span class="tag">Linux x86_64</span><h3>Linux</h3><p>Portable x86_64 tarball for current Linux distributions.</p><div class="file-name" data-release-file="linux"><?= htmlspecialchars($files['linux'], ENT_QUOTES, 'UTF-8') ?></div><div class="file-meta" data-release-meta="linux" hidden></div><div class="actions"><a class="button peach-button" data-release-link="linux" href="<?= $files['linux'] !== '' ? 'file/' . rawurlencode($files['linux']) : $releasesUrl ?>">Download Linux</a></div>
<p class="download-alternative" data-release-optional="linux_glibc_duckdb"<?= $files['linux_glibc_duckdb'] === '' ? ' hidden' : '' ?>><small><a data-release-link="linux_glibc_duckdb" href="<?= $files['linux_glibc_duckdb'] !== '' ? 'file/' . rawurlencode($files['linux_glibc_duckdb']) : $releasesUrl ?>">Download with DuckDB (glibc)</a></small></p></div>
...
<div class="changelog">
<h2>Release notes</h2>
<p>See <a data-release-notes href="<?= $release !== '' ? $releasesUrl . '/tag/' . rawurlencode($release) : $releasesUrl ?>">notes for the current release</a> and <a href="<?= $releasesUrl ?>">all releases</a> for changes and earlier versions.</p>
</div>
</div></section></main>
