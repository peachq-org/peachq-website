...
peachq_source: user-docs/syscmds.md
...
listed behave as the reference body further down describes.
...
| `\` (bare) | **differs** — a silent no-op: there is no k interpreter to toggle into. |
| `\r` | **differs** — both forms (replication primary; rename) are accepted silently but do nothing, and the getter answers `'nyi`. |
| `\T` | **differs** — `\T 30` is accepted silently but not applied; the getter answers `'nyi`. |
| `\u` | **differs** — the bare reload answers `'nyi`; `\u file` is accepted silently. |
| `\e \g \o \s \W` | **partly** — get and set round-trip the value, but the behaviour behind the number (error trapping, real collection, the UTC offset reaching `.z.Z`, thread re-tuning, the week start) is not wired yet. |
| everything else | **answers as documented at the command surface** — `\a \b \B \c \cd \C \d \f \l \p \P \S \t \ts \v \w \z \E \1 \2 \\` and the unknown-token OS escape. This table is re-derived per command, not a clause-by-clause conformance audit of the reference body below. |
...
> **Tip: An alternative method to executing system commands is to use the [`system`](https://code.kx.com/q/ref/system/) keyword. This executes a string representation of a system command and returns its result.**
...
[`.z.b`](https://code.kx.com/q/ref/dotz/#zb-dependencies)
<br>
...
[`-c` command-line option](https://code.kx.com/q/basics/cmdline/#-c-console-size)
...
[`-C` command-line option](https://code.kx.com/q/basics/cmdline/#-c-http-size)
...
[Command-line option `-e`](https://code.kx.com/q/basics/cmdline/#-e-error-traps)
...
[Command-line option `-E`](https://code.kx.com/q/basics/cmdline/#-e-tls-server-mode) to set the mode
...
Show or set garbage-collection mode. The default mode is 0 (deferred). Setting the garbage-collection mode will automatically call [`.Q.gc[]`](https://code.kx.com/q/ref/dotq/#gc-garbage-collect) after setting the provided value.
...
If the subsequent attempt fail, the request exits with [`'wsfull`](https://code.kx.com/q/basics/errors/#wsfull).

> **Detail: Notes on the allocator**
>
> Q’s allocator bins objects in power-of-two size categories, from 16b (e.g. an atom) to 64MB.
>
> In this example, various vectors of longs (8 bytes per long) are created of different sizes using [`til`](https://code.kx.com/q/ref/til/).
...
The following example shows freeing an object ≥64MB in `deferred` mode, while inspecting memory usage via [`.Q.w[]`](https://code.kx.com/q/ref/dotq/#w-memory-stats):
```q
...
No memory is returned to the OS on freeing the objects, and only when [`.Q.gc[]`](https://code.kx.com/q/ref/dotq/#gc-garbage-collect) is run is the memory coalesced and freed.
```q
q).Q.w[]`used`heap               / original memory used and memory reserved by kdb+ at time of test
371648 67108864
...
[Command-line option `-g`](https://code.kx.com/q/basics/cmdline/#-g-garbage-collection) (garbage collection mode), [Command-line parameter `-w`](https://code.kx.com/q/basics/cmdline/#-w-workspace) (workspace memory limit), [System command `\w`](https://code.kx.com/q/basics/syscmds/#w-workspace) (memory stats and workspace memory limit)
<br>
_Q for Mortals_
...
If [logging](https://code.kx.com/q/kb/logging/) is enabled, the command [checkpoints](https://code.kx.com/q/kb/logging/#check-pointing-rolling) the `.qdb` file and empties the log file.
...
[`.Q.l`](https://code.kx.com/q/ref/dotq/#l-load) (load)
<br>
[Logging](https://code.kx.com/q/kb/logging/)
<br>
_Q for Mortals_
...
[Command-line option `-o`](https://code.kx.com/q/basics/cmdline/#-o-utc-offset)
...
See
[Listening port](https://code.kx.com/q/basics/listening-port/) for detail.

[`hopen`](https://code.kx.com/q/ref/hopen/)
<br>
[`-p` command-line option ](https://code.kx.com/q/basics/cmdline/#-p-listening-port)
<br>
[Multithreaded input mode](https://code.kx.com/q/basics/listening-port/#multi-threaded-input-mode),
[Changes in 3.5](https://code.kx.com/q/releases/ChangesIn3.5/#socket-sharding)
<br>
...
`\P` is applied when [save](https://code.kx.com/q/ref/save/) exports to text files (CSV, JSON, etc.) and whenever a floating-point number is converted to a string.
...
[`.Q.f`](https://code.kx.com/q/ref/dotq/#f-precision-format) (precision format),
[`.Q.fmt`](https://code.kx.com/q/ref/dotq/#fmt-precision-format) (precision format with length)
<br>
[Precision](https://code.kx.com/q/basics/precision/),
[`-P` command-line option](https://code.kx.com/q/basics/cmdline/#-p-display-precision),
[`-27!` precision format with IEEE754 rounding](https://code.kx.com/q/basics/internal/#-27xy-ieee754-precision-format)
<br>
...
[`-r` command-line option](https://code.kx.com/q/basics/cmdline/#-r-replicate)

[Replication](https://code.kx.com/q/kb/logging/#replication)
...
Show or , where `N` is an integer, set the number of secondary threads available for parallel processing, within the limit set by the [`-s` command-line option](https://code.kx.com/q/basics/cmdline/#-s-secondary-threads).
`N` is an integer.
...
-   `peach` or `':` will call [`.z.pd`](https://code.kx.com/q/ref/dotz/#zpd-peach-handles) for a list of handles to the processes, which must have been started previously
-   the absolute value of `-N` in the command line is ignored

[`-s` command-line option](https://code.kx.com/q/basics/cmdline/#-s-secondary-threads),
...
: Set the number of milliseconds between timer ticks. If 0, the timer is disabled, otherwise the timer is enabled and the first tick given. On each tick, the function assigned to [`.z.ts`](https://code.kx.com/q/ref/dotz/#zts-timer) is executed.

: This usage corresponds to the [`-t` command-line option](https://code.kx.com/q/basics/cmdline/#-t-timer-ticks)
...
[`-T` command-line option](https://code.kx.com/q/basics/cmdline/#-t-timeout)
...
[`-u` command-line option](https://code.kx.com/q/basics/cmdline/#-u-usr-pwd-local)
...
The utility [`.Q.w`](https://code.kx.com/q/ref/dotq/#w-memory-stats) formats all this information.

**Run-time increase**
Since 2017.11.06, `\w` allows the workspace limit to be increased at run-time, if it was initialized via the
[`-w` command-line option](https://code.kx.com/q/basics/cmdline/#-w-workspace). For example `\w 128` sets the limit to 128MB if the `-w` command line option was specified
...
**Domain-local**
Since V4.0 2020.03.17 returns information for the [current memory domain](https://code.kx.com/q/ref/dotm/#memory-backed-files) only.
...
[`-w` workspace command-line option](https://code.kx.com/q/basics/cmdline/#-w-workspace), [`\g`](#g-garbage-collection-mode) (garbage-collection mode)<br>
[`.m` namespace](https://code.kx.com/q/ref/dotm/#memory-backed-files) (DAX-enabled filesystems)
...
[`-W` command-line option](https://code.kx.com/q/basics/cmdline/#-w-start-week)
...
[`-z` command-line option](https://code.kx.com/q/basics/cmdline/#-z-date-format)
...
[`-b` command-line option](https://code.kx.com/q/basics/cmdline/#-b-blocked)
...
[Protecting proprietary code](https://code.kx.com/q/wp/permissions/index/#protecting-proprietary-code)
...
[`.z.exit`](https://code.kx.com/q/ref/dotz/#zexit-action-on-exit)
